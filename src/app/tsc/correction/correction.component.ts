...
export class CorrectionComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.route.queryParams.subscribe((params) => {
      ...
      const organizationName = this.route.snapshot.queryParamMap.get('organizationName') || 'Unknown Organization';
  const schoolName = this.route.snapshot.queryParamMap.get('schoolName') || 'Unknown School';
  const teacherName = this.route.snapshot.queryParamMap.get('teacherName') || 'Unknown Teacher';
  ...
  updateBreadcrumb(organizationName: string, schoolName: string, teacherName: string) {
    this.breadcrumb = [
      { name: organizationName, link: '/tsc-navbar/assign-organization' },
      { name: schoolName, link: '/tsc-navbar/assign-school' },
      { name: teacherName, link: '/tsc-navbar/assign-teacher' },
      { 
  ...
  submitScores() {
    ...
    const postScore$ =
      this.projectType === 'teacher'
        ? this.correctionService.postTeacherProjectScore(projectId, this.userId, scoreData)
        : this.correctionService.postStudentProjectScore(projectId, this.userId, scoreData);
  ...
  updateScore() {
    ...
    const patchScore$ =
      this.projectType === 'teacher'
        ? this.correctionService.patchTeacherProjectScore(projectId, this.userId, scoreData)
        : this.correctionService.patchStudentProjectScore(projectId, this.userId, scoreData);
