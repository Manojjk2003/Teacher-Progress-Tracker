...
// Define a response model (adjust fields according to your API response)
...
//   teacher_project_links: { project_id: string; project_link: string }[];
//   student_project_links: { project_id: string; project_link: string }[];
// }
...
export class AdminTeacherdashboardService {
  private baseUrl = 'http://api.kiddypi.com:8001'; // Replace with your actual backend URL

  constructor(private http: HttpClient) {}

  getDashboard(teacherId: string): Observable<DashboardResponse> {
    return this.http.get<DashboardResponse>(`${this.baseUrl}/dashboard/${teacherId}`);
  }
